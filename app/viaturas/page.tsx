...
export default async function ViaturasPage() {
  ...
  const active = (activeVehicleBooking as any)?.data;
  ...
  return (
    <main className="relative mx-auto w-full max-w-6xl flex-1 px-4 pb-16 pt-6 md:px-6 md:pb-20 md:pt-8">
        ...
        <section className="relative w-full overflow-hidden rounded-3xl bg-gradient-to-br from-[#0f3560] via-[#143a6b] to-[#1D4E89] px-5 py-10 text-white shadow-xl ring-1 ring-white/10 md:px-10 md:py-12">
          ...
          {vehicles.length > 0 ? (
            <div className="relative z-10 mt-8 grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  key={s.label}
                  className="flex items-start gap-3 rounded-2xl border border-white/15 bg-white/10 p-4 ring-1 ring-white/10 backdrop-blur-sm"
                >
        ...
        {active?.end_date && active?.vehicles?.model ? (
          <VehicleActiveBookingBanner
            vehicleModel={active.vehicles.model}
        ...
        <div className="mt-8 md:mt-10">
          <section className="rounded-3xl border border-white/15 bg-white p-6 shadow-2xl shadow-black/20 ring-1 ring-white/20 md:p-8 lg:p-10">
            <div className="flex flex-col gap-6 border-b border-zinc-100 pb-8 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Link
                ...
                className="inline-flex shrink-0 items-center justify-center gap-2 self-start rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-2.5 text-sm font-semibold text-[color:var(--brand-900)] transition hover:border-[color:var(--brand-500)]/40 hover:bg-white sm:self-end"
              >
                Ir para reserva
